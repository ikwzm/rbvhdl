...
describe 'RbVHDL::Ast::Statement::Case' do
  ...
  context "_when" do
    ...
    it "RbVHDL::Ast.case_statement(nil, name('curr_state'))._when(choice_identifier('IDLE_STATE')) ... _when(choice_others)" do
      ...
      idle_state  = stmt._when(RbVHDL::Ast.choice_identifier('IDLE_STATE'))
      idle_state._null_statement
      other_state = stmt._when(RbVHDL::Ast.choice_others)
      other_state._null_statement
      ...
      expect(stmt._when_list[0]._statement_list[0]._label).to eq nil
      ...
      expect(stmt._when_list[1]._statement_list[0]._label).to eq nil
    end
